...
import {
    ...
    DialogTitle, Divider, FormControl, FormControlLabel, Grid, InputAdornment,
...
const EMPTY_FORM = {
    name: '', description: '', usageType: 'Whole Item', unitOfMeasure: '', isActive: true,
    defaultStockQty: '', lowStockThreshold: '', defaultCostPrice: '', purchaseQty: '',
};
...
export default function MaterialTypeModal({ open, initial, onClose, onSaved }) {
    ...
    const unitSuffix = form.usageType === 'Percentage'
        ? (form.unitOfMeasure || '')
        : 'pcs';
    ...
    const stockAdornment = unitSuffix
        ? { endAdornment: <InputAdornment position="end">{unitSuffix}</InputAdornment> }
        : undefined;

    return (
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
            <DialogTitle>
                {isEdit ? t('settings.materialTypes.modal.editTitle') : t('settings.materialTypes.modal.createTitle')}
            </DialogTitle>
            <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: '12px !important' }}>
                ...
                <FormControl fullWidth required>
                    ...
                    <Select
                        ...
                        label={t('settings.materialTypes.modal.usageTypeLabel')}
                        onChange={set('usageType')}
                    >
                        {USAGE_TYPES.map((u) => (
                            <MenuItem key={u} value={u}>{t(`usageTypes.${USAGE_I18N[u]}`)}</MenuItem>
                        ))}
                    </Select>
                </FormControl>
                ...
                {form.usageType === 'Percentage' && (
                    <FormControl fullWidth required error={!form.unitOfMeasure}>
                        ...
                        <Select
                            ...
                            label={`${t('settings.materialTypes.modal.unitLabel')} *`}
