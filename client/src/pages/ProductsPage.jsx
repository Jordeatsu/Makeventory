...
export default function ProductsPage() {
    ...
    const groupedByType = useMemo(() => {
        ...
        return ["Standard", "Template", "Variant"].map((type) => ({ type, items: groups[type] })).filter((g) => g.items.length > 0);
    }, [products]);
    ...
    const tableHead = (
        <TableHead>
            <TableRow sx={{ "& th": { fontWeight: 600, bgcolor: "background.default" } }}>
                ...
                {col("sku") && <TableCell>{t("products.col.sku")}</TableCell>}
                {col("category") && <TableCell>{t("products.col.category")}</TableCell>}
                {col("estMaterialCost") && <TableCell align="right">{t("products.col.estMaterialCost")}</TableCell>}
                {col("basePrice") && <TableCell align="right">{t("products.col.basePrice")}</TableCell>}
                {col("estMargin") && <TableCell align="right">{t("products.col.estMargin")}</TableCell>}
                {col("status") && <TableCell>{t("products.col.status")}</TableCell>}
    ...
    const renderRow = (p) => {
        ...
        const margin = p.basePrice > 0 ? (((p.basePrice - totalCost) / p.basePrice) * 100).toFixed(1) : null;
        return (
            <TableRow key={p._id} hover sx={{ cursor: "pointer" }} onClick={() => navigate(`/products/${p._id}`)}>
                ...
                {col("sku") && <TableCell sx={{ color: "text.secondary" }}>{p.sku || "—"}</TableCell>}
                {col("category") && <TableCell>{p.category || "—"}</TableCell>}
                {col("estMaterialCost") && <TableCell align="right">{fmt(totalCost)}</TableCell>}
                {col("basePrice") && <TableCell align="right">{fmt(p.basePrice)}</TableCell>}
                {col("estMargin") && (
    ...
    return (
        <Box>
            ...
            <Dialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} maxWidth="xs" fullWidth>
                ...
                <DialogContent>
                    <Typography>{t("products.delete.confirm", { name: deleteTarget?.name })}</Typography>
                </DialogContent>
