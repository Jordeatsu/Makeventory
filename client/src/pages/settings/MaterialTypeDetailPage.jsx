...
export default function MaterialTypeDetailPage() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setError(e.response?.status === 404
                ? t("settings.materialTypes.notFound", "Material type not found.")
                : t("settings.materialTypes.loadError", "Failed to load material type."));
        } finally {
            setLoading(false);
        }
    }, [id, t]);
    ...
    const hasDefaults = mt.defaultStockQty != null || mt.lowStockThreshold != null
        || mt.defaultCostPrice != null || mt.purchaseQty != null;
        ...
        <Box sx={{ maxWidth: 860, mx: "auto" }}>
            ...
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={3} flexWrap="wrap" gap={1}>
                <Stack direction="row" alignItems="center" gap={1}>
                    ...
                    <Box>
                        <Stack direction="row" alignItems="center" gap={1}>
                            <Typography variant="h5" fontWeight={600}>{mt.name}</Typography>
                            <Chip
                        ...
                        {mt.description && (
                            <Typography variant="body2" color="text.secondary" mt={0.25}>{mt.description}</Typography>
                        )}
                    </Box>
                </Stack>
            ...
            <Grid container spacing={2} mb={3}>
                <Grid item xs={12} sm={4}>
                    <Paper variant="outlined" sx={{ p: 2, textAlign: "center" }}>
                        ...
                        <Typography variant="h6" fontWeight={700} color="primary.main">
                            {USAGE_KEY[mt.usageType]
                                ? t(`usageTypes.${USAGE_KEY[mt.usageType]}`)
                                : mt.usageType}
                        </Typography>
                    </Paper>
                </Grid>
                <Grid item xs={12} sm={4}>
                    <Paper variant="outlined" sx={{ p: 2, textAlign: "center" }}>
                        ...
                        <Typography variant="h6" fontWeight={700}>{unitLabel}</Typography>
                    </Paper>
                </Grid>
                <Grid item xs={12} sm={4}>
                    <Paper variant="outlined" sx={{ p: 2, textAlign: "center" }}>
                        ...
                        <Typography
                            variant="h6"
                            fontWeight={700}
                            color={mt.isActive ? "success.main" : "text.secondary"}
                        >
            ...
            {hasDefaults && (
                <Paper variant="outlined" sx={{ p: 3, mb: 3 }}>
                    ...
                    <Grid container spacing={2}>
                        <DetailRow
                            label={t("settings.materialTypes.modal.defaultStockQty", "Default Stock Qty")}
                            value={mt.defaultStockQty != null ? `${mt.defaultStockQty} ${unitLabel}` : null}
                        />
                        <DetailRow
                            label={t("settings.materialTypes.modal.lowStockThreshold", "Low Stock Threshold")}
                            value={mt.lowStockThreshold != null ? `${mt.lowStockThreshold} ${unitLabel}` : null}
                        />
                        <DetailRow
                            label={t("settings.materialTypes.modal.defaultCostPrice", "Default Cost Price")}
                            value={mt.defaultCostPrice != null ? `${currencySymbol}${Number(mt.defaultCostPrice).toFixed(2)}` : null}
                        />
                        <DetailRow
                            label={t("settings.materialTypes.modal.purchaseQty", "Purchase Qty")}
                            value={mt.purchaseQty != null ? `${mt.purchaseQty} ${unitLabel}` : null}
                        />
                    </Grid>
                </Paper>
            )}
                ...
                <Grid container spacing={2}>
                    {mt.createdAt && (
                        <Grid item xs={12} sm={6}>
                            ...
                            {mt.createdBy?.name && (
                                <Typography variant="body1" fontWeight={600}>{mt.createdBy.name}</Typography>
                            )}
                    ...
                    {mt.updatedAt && (
                        <Grid item xs={12} sm={6}>
                            ...
                            {mt.updatedBy?.name && (
                                <Typography variant="body1" fontWeight={600}>{mt.updatedBy.name}</Typography>
                            )}
                            <Typography variant="body2" color="text.secondary">
