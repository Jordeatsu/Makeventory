...
import { Box, Typography, Paper, Switch, Divider, Button, CircularProgress, Alert, Skeleton } from "@mui/material";
...
export default function ModuleSelectionPage() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Notify SettingsLayout so it re-filters the nav without a full reload
        ...
        <Box sx={{ maxWidth: 600, mx: "auto" }}>
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 3 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                    ...
                    <Typography variant="h5" fontWeight={600}>
                        {t("settings.moduleSelection.title")}
                    </Typography>
                </Box>
                <Button variant="contained" startIcon={saving ? <CircularProgress size={16} color="inherit" /> : <SaveIcon />} onClick={handleSave} disabled={!isDirty || saving}>
            ...
            {success && (
                <Alert severity="success" sx={{ mb: 2 }}>
                    {t("settings.moduleSelection.saved")}
                </Alert>
            )}
                    ...
                    : modules.map((m, i) => (
                          <React.Fragment key={m._id}>
                              ...
                              <Box sx={{ px: 3, py: 1.5, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                  ...
                                  <Switch checked={draft[m._id] ?? false} onChange={(e) => setDraft((prev) => ({ ...prev, [m._id]: e.target.checked }))} color="primary" />
                              </Box>
                          </React.Fragment>
                      ))}
            </Paper>
