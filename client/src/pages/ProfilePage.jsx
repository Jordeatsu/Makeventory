...
import { Box, Typography, Avatar, Divider, Paper, Grid, Chip, CircularProgress, Alert, Button, Dialog, DialogTitle, DialogContent, DialogActions, TextField, IconButton, InputAdornment, Snackbar } from "@mui/material";
...
const fmt = (dateStr) => (dateStr ? new Date(dateStr).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" }) : "—");

const getInitials = (p) => (p ? `${p.firstName?.[0] ?? ""}${p.lastName?.[0] ?? ""}`.toUpperCase() : "?");
...
export default function ProfilePage() {
    ...
    if (error)
        return (
            <Alert severity="error" sx={{ maxWidth: 700, mx: "auto", mt: 4 }}>
                {error}
            </Alert>
        );
            ...
            <Paper elevation={0} sx={{ borderRadius: 3, overflow: "hidden", mb: 3, border: "1px solid", borderColor: "divider" }}>
                ...
                >
                    ...
                    <Box sx={{ flex: 1 }}>
                        ...
                        <Chip label={profile?.role} size="small" sx={{ bgcolor: "rgba(255,255,255,0.2)", color: "white", fontWeight: 600, textTransform: "capitalize", mb: 1 }} />
            ...
            {canEdit && (
                <Box sx={{ mb: 3 }}>
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid size={{ xs: 12, sm: isOwnProfile ? 6 : 12 }}>
                            <Paper variant="outlined" sx={{ p: 3, borderRadius: 2, transition: "box-shadow 0.2s", "&:hover": { boxShadow: 3 } }}>
                                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 1.5 }}>
                ...
                {detailRows.map(({ icon, label, value, mono }, i) => (
                    <React.Fragment key={label}>
                        ...
                        <Box sx={{ px: 3, py: 2, display: "flex", alignItems: "center", gap: 2 }}>
                            ...
                            <Typography variant="body2" fontWeight={500} sx={{ fontFamily: mono ? "monospace" : undefined, fontSize: mono ? "0.8rem" : undefined, wordBreak: "break-all" }}>
                                {value ?? "—"}
                            </Typography>
                        </Box>
                    </React.Fragment>
            ...
            <Dialog open={editOpen} onClose={() => !editLoading && setEditOpen(false)} maxWidth="sm" fullWidth>
                ...
                <Box component="form" onSubmit={handleEditSubmit}>
                    <DialogContent sx={{ pt: 1 }}>
                        ...
                        <Grid container spacing={2}>
                            <Grid size={{ xs: 12, sm: 6 }}>
                                <TextField label={t("profile.firstName")} fullWidth required value={editForm.firstName} onChange={(e) => setEditForm((f) => ({ ...f, firstName: e.target.value }))} />
                            </Grid>
                            <Grid size={{ xs: 12, sm: 6 }}>
                                <TextField label={t("profile.lastName")} fullWidth required value={editForm.lastName} onChange={(e) => setEditForm((f) => ({ ...f, lastName: e.target.value }))} />
                            </Grid>
                            <Grid size={12}>
                                <TextField label={t("profile.username")} fullWidth required value={editForm.username} onChange={(e) => setEditForm((f) => ({ ...f, username: e.target.value }))} />
                            </Grid>
                            <Grid size={12}>
                                <TextField label={t("profile.email")} type="email" fullWidth required value={editForm.email} onChange={(e) => setEditForm((f) => ({ ...f, email: e.target.value }))} />
                            </Grid>
                        </Grid>
                    </DialogContent>
                    <DialogActions sx={{ px: 3, pb: 2 }}>
