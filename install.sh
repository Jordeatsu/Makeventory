...
for i in $(seq 1 15); do
    if node -e "
        const http = require('http');
            http.get('http://localhost:3000/api/ping', () => process.exit(0))
...
# ─── Open browser ────────────────────────────────────────────────────────────
echo "Opening installer in your browser at http://localhost:3000"
open_browser "http://localhost:3000"

# Keep script alive until the installer server exits (user completes setup)
wait $SERVER_PID
echo ""
echo "Installation complete. Now starting Makeventory in production mode..."
