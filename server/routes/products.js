...
router.get('/products', requireAuth, async (req, res) => {
    try {
        ...
        const docs = await Product.find(filter)
            .populate('parentProduct', 'name estimatedMaterialCost defaultMaterials')
            .sort({ name: 1 })
            .lean();
...
router.get('/products/:id/stats', requireAuth, async (req, res) => {
    try {
        ...
        const product = await Product.findById(id)
            .populate('parentProduct', 'name defaultMaterials estimatedMaterialCost')
...
router.post('/products', requireAuth, async (req, res) => {
    try {
        const {
            name, sku, category, description, basePrice, active, isTemplate,
            parentProduct, defaultMaterials,
        } = req.body ?? {};
        if (!name?.trim()) return res.status(400).json({ error: 'Name is required.' });

        const estimatedMaterialCost = (defaultMaterials || []).reduce(
            (s, m) => s + (m.lineCost || 0), 0,
        );
        const doc = await Product.create({
            name: name.trim(), sku, category, description,
