...
router.get('/year-review/stats/:year', requireAuth, async (req, res) => {
    ...
    try {
                ...
                },
                {
                    $addFields: {
                        totalRevenue: {
                            $subtract: [
                                '$totalGrossRevenue',
                                { $add: ['$totalShipping', '$totalBuyerTax', '$totalRefunds'] },
                            ],
                        },
                        avgOrderValue: {
                            $cond: [
                                { $gt: ['$totalOrders', 0] },
                                { $divide: ['$totalGrossRevenue', '$totalOrders'] },
                                0,
                            ],
                        },
                        avgProfit: {
                    ...
                    $group: {
                        ...
                        revenue: {
                            $sum: {
                                $subtract: [
                                    '$totalCharged',
                                    { $add: ['$shippingCost', '$buyerTax', '$refund'] },
                                ],
                            },
                        },
                        profit: { $sum: '$profit' },
        ...
        const materials = [...orderMaterials, ...overheadMapped].sort(
