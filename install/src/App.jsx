...
import {
  Box, Divider, List, ListItem, ListItemIcon,
      ...
      <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
          ...
          <List sx={{ px: 1, py: 1.5, flexGrow: 1 }}>
            {STEPS.map(({ id, label }) => {
              ...
              return (
                <ListItem
                  key={id}
                  onClick={() => { if (status !== 'pending') setCurrentStep(id); }}
                  sx={{
                    borderRadius: 2,
                    mb: 0.5,
        ...
        >
          {currentStep === 'dependencies' && (
            <DependenciesStep
              alreadyComplete={stepStatus.dependencies === 'complete'}
              onComplete={() => markComplete('dependencies')}
            />
          )}

          {currentStep === 'database' && (
            <DatabaseStep
              savedDbName={savedDbName}
              onDbNameSaved={(name) => setSavedDbName(name)}
              onComplete={() => markComplete('database')}
              onError={()    => markError('database')}
            />
          )}

          {currentStep === 'account' && (
            <AccountStep
              savedData={savedAccount}
              onSave={(data) => setSavedAccount(data)}
              onComplete={() => markComplete('account')}
            />
          )}
          {currentStep === 'business' && (
            <BusinessStep
              savedData={savedBusiness}
              onSave={(data) => setSavedBusiness(data)}
              onComplete={() => markComplete('business')}
            />
          )}
          {currentStep === 'modules' && (
            <Box>
              <Typography variant="h5" gutterBottom>Module Selection</Typography>
