...
import {
  ...
  ListItemIcon, ListItemText, Alert, TextField,
...
export default function DatabaseStep({ onComplete, onError, savedDbName, onDbNameSaved }) {
  ...
  const addLine = (message, status = S.INFO) =>
    setLog((prev) => [...prev, { id: Date.now() + Math.random(), message, status }]);
  ...
  const runDockerFlow = async () => {
    ...
    const { data: { exists, running } } = await api.checkDockerContainer();
  ...
  const handleCreate = async () => {
    ...
    if (err) { setDbNameError(err); return; }
  ...
  return (
    <Box>
      <Typography variant="h5" gutterBottom>Database Setup</Typography>
      ...
      {phase === 'hard_stop' && (
        <Alert severity="error" sx={{ mb: 2 }}>
          ...
          MongoDB was not found on your system and Docker is not available. Please install one of
          the following, then re-run <code style={{ fontFamily: 'monospace' }}>./install.sh</code>:
          <ul style={{ marginTop: 8 }}>
            <li>
              <strong>MongoDB Community Edition</strong> —{' '}
        ...
        <Fade in>
          <Box>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Choose a name for your Makeventory database. Letters only (e.g.{' '}
              <em>makeventory</em>).
            </Typography>
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start', maxWidth: 440 }}>
              ...
              <Button
                ...
                disabled={creating || !!dbNameError || !dbName}
      ...
      {phase === 'complete' && (
        <Alert severity="success">
          Database setup complete — moving to the next step…
        </Alert>
      )}
    </Box>
  );
