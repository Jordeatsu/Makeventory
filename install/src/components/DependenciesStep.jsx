    ...
    return (
        <Paper
            ...
            sx={{
                p: 2,
                mb: 1.5,
                bgcolor: isComplete ? "rgba(52,211,153,0.05)" : isError ? "rgba(248,113,113,0.05)" : "rgba(255,255,255,0.02)",
                borderColor: isComplete ? "rgba(52,211,153,0.3)" : isError ? "rgba(248,113,113,0.3)" : isRunning ? "rgba(96,165,250,0.3)" : "rgba(255,255,255,0.08)",
                borderRadius: 2,
        ...
        >
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 1.2 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                    ...
                    <Typography variant="caption" color="text.disabled" sx={{ fontFamily: '"Roboto Mono", monospace', fontSize: "0.7rem" }}>
                        {desc}
                    </Typography>
                </Box>
                ...
                {isRunning && <Chip label="Installing…" size="small" color="primary" variant="outlined" sx={{ fontSize: "0.7rem", height: 20 }} />}
                {status === "pending" && <Chip label="Waiting" size="small" variant="outlined" sx={{ fontSize: "0.7rem", height: 20, borderColor: "text.disabled", color: "text.disabled" }} />}
            </Box>

            <LinearProgress variant={isRunning ? "indeterminate" : "determinate"} value={isRunning ? undefined : isComplete ? 100 : 0} color={isComplete ? "success" : isError ? "error" : "primary"} sx={{ borderRadius: 4, height: 6 }} />
    ...
    return (
        <Box>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                npm packages are being installed for each part of Makeventory. This usually takes under a minute.
            </Typography>

            {PACKAGES.map(({ id, label, desc }) => (
                <PackageRow key={id} label={label} desc={desc} status={jobs[id]?.status ?? "pending"} />
            ))}
            ...
            {anyError && (
                <Alert severity="error" sx={{ mt: 2 }}>
                    One or more packages failed to install. Check your network connection and re-run <code style={{ fontFamily: "monospace" }}>./install.sh</code>.
                </Alert>
            )}
        </Box>
    );
