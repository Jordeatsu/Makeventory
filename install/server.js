    ...
    if (mongoAvailable) {
      if (OS === 'mac') {
        ...
        await runCommand(
          ...
          'mongod --fork --logpath /tmp/makeventory-mongod.log --dbpath /usr/local/var/mongodb 2>/dev/null'
        );
      } else if (OS === 'linux') {
        await runCommand(
...
app.post('/api/account/create', async (req, res) => {
  ...
  const missing = ['firstName', 'lastName', 'email', 'username', 'password']
...
app.put('/api/account/update', async (req, res) => {
  ...
  const missing = ['firstName', 'lastName', 'email', 'username']
    .filter((k) => !req.body?.[k]?.trim());
  if (missing.length) {
    return res.status(400).json({ error: `Missing required fields: ${missing.join(', ')}` });
  }
