...
echo "🛠  Starting Makeventory (development)..."
...
# ── API Server (dev: nodemon with hot-reload) ─────────────────────────────────
echo "  → Starting API server in dev mode (port 5001)..."
cd "$ROOT_DIR/server"
if command -v nohup &>/dev/null; then
    nohup env PORT=5001 NODE_ENV=development npm run dev > "$ROOT_DIR/logs/server.log" 2>&1 &
else
    env PORT=5001 NODE_ENV=development npm run dev > "$ROOT_DIR/logs/server.log" 2>&1 &
fi
echo $! > "$ROOT_DIR/logs/server.pid"

# ── Client (dev: vite HMR) ────────────────────────────────────────────────────
echo "  → Starting client in dev mode (port 3000)..."
cd "$ROOT_DIR/client"
if command -v nohup &>/dev/null; then
    nohup npm run dev > "$ROOT_DIR/logs/client.log" 2>&1 &
else
    npm run dev > "$ROOT_DIR/logs/client.log" 2>&1 &
fi
echo $! > "$ROOT_DIR/logs/client.pid"

echo ""
echo "✅ Makeventory is running (development)!"
echo "   API:  http://${IP}:5001/api"
echo "   App:  http://${IP}:3000"
echo ""
echo "   Logs: logs/server.log | logs/client.log"
echo "   Run ./systemfiles/dev/stop.sh to shut everything down."
